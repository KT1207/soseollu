...
const AddNovelImg = () => {
  ...
  const postNovel = () => {
    if (
      ...
      axios
        .post("http://localhost:8001/post/AdminAddImg", formData, {
          headers: {
    ...
    <div>
      <div>
        ...
        <input
          type="text"
          maxLength={100}
          placeholder="신청하고자 하는 작품의 제목을 적어주세요"
          ref={NovelNameRef}
        />
      </div>
      <div>
        ...
        <textarea
          ...
          rows={rows}
          maxLength={2000}
          placeholder="작품 소개를 써주세요"
          ref={NovelIntroduceRef}
        ></textarea>
      </div>
      <div>
        <div>소설 배너 이미지 선택</div>
        <input
          type="file"
          accept="image/*"
      ...
      <div>
        <button onClick={postNovel}>신청하기</button>
      </div>
    </div>
  );
};

export default AddNovelImg;
